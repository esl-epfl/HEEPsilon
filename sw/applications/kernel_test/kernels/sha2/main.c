...
int main(void) {
    ...
    for(int r=0 ; r < REPETITIONS ; r++){
		...
		int32_t W_sw[20];
		for(int i=0 ; i < 20 ; i++)
			W_sw[i] = W[i] = rand() % (UINT_MAX - 1 - 0 + 1) + 0;
        ...
        int32_t res_cgra[5];
		res_cgra[1] = cgra_output[1][0];
		res_cgra[0] = cgra_output[1][1];
		res_cgra[2] = cgra_output[3][0];
		res_cgra[3] = cgra_output[3][1];
		res_cgra[4] = cgra_output[3][2];
		for(int i=0 ; i < 5 ; i++)
		{
			if (res_cgra[i] != res_sw[i])
				errors++;
		}
